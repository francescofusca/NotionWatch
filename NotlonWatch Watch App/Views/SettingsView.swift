//  NotlonWatch/Views/SettingsView.swift
import SwiftUI
import WatchKit
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    settingsSection(
                        title: "Impostazioni Trascrizione",
                        icon: "text.bubble.fill",
                        iconColor: .green
                    ) {
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                Toggle("Abilita trascrizione", isOn: $viewModel.enableTranscription)
                                    .toggleStyle(SwitchToggleStyle(tint: .green))
                            }

                            if viewModel.enableTranscription {
                                HStack {
                                    Toggle("Trascrizione automatica", isOn: $viewModel.enableAutoTranscription)
                                        .toggleStyle(SwitchToggleStyle(tint: .blue))
                                        .disabled(!viewModel.enableTranscription)
                                }

                                if viewModel.enableAutoTranscription {
                                    Text("L'audio sarà trascritto automaticamente usando Speech Recognition")
                                        .font(.caption2)
                                        .foregroundColor(.secondary)
                                } else {
                                    Text("Dovrai inserire manualmente la descrizione dopo la registrazione")
                                        .font(.caption2)
                                        .foregroundColor(.secondary)
                                }
                            } else {
                                Text("Sarà inviato solo l'audio senza trascrizione")
                                    .font(.caption2)
                                    .foregroundColor(.secondary)
                            }
                        }
