//  NotlonWatch/Services/NetworkService.swift
import Foundation
import Combine

class NetworkService {
    func uploadAudio(fileURL: URL, description: String, transcription: String?, apiKey: String, databaseId: String, cloudinaryApiKey: String, cloudinaryApiSecret: String, cloudinaryCloudName: String, completion: @escaping (Result<String, Error>) -> Void) {

        guard let url = URL(string: AppConfig.serverURL) else { // Configuration located in AppConfig.swift
        ...
        if let transcription = transcription {
            data.append("--\(boundary)\r\n".data(using: .utf8)!)
            data.append("Content-Disposition: form-data; name=\"transcription\"\r\n\r\n".data(using: .utf8)!)
            data.append(transcription.data(using: .utf8)!)
            data.append("\r\n".data(using: .utf8)!)
        }
